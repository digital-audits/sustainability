...
export default class CollectScreenshot extends Collect {
	...
	static async collect(
        ...
        const  rgbPower:RGBPowerFormat = {r:RED_CHANNEL_POWER, g:GREEN_CHANNEL_POWER, b:BLUE_CHANNEL_POWER}
        //wait until domcontentloaded to avoid screenshot of cookie banner
        await util.safeNavigateTimeout(page, 'domcontentloaded', settings.maxNavigationTime)
        ...
        await Promise.race([
            ...
            util.safeNavigateTimeout(darkModePage, 'domcontentloaded', settings.maxNavigationTime)])

        const base64ScreenshotDark = await darkModePage.screenshot({encoding:'base64'})
        await Promise.all([
            page.bringToFront(),
            darkModePage.close()
        ])
        
        const RGBPixelPower = async (screenshot:string, rgbPower:RGBPowerFormat) => await page.evaluate(async function(base64Screenshot, rgbPower){
            
            
            let image = new Image()
            let imgLoadPromise = async function(){ return new Promise((resolve, reject) => {
                image.onload = () => {debug('Image successfully loaded into DOM'); return resolve};
          ...
          //  console.log('DARK',window.matchMedia('(prefers-color-scheme: light)').matches
            ...
            return average.pixels*(average.r*pixelPower.r + average.b*pixelPower.b 
                + average.g*pixelPower.g) / 1000000


        }, screenshot, JSON.stringify(rgbPower))
        ...
        const hasDarkMode = shPromises[1] < shPromises[0] && 1- (Math.abs(shPromises[0] - shPromises[1])/ Math.max(...shPromises)) <= SCREENSHOT_SIMILARITY_THRESHOLD
