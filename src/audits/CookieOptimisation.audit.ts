...
export default class CookieOptimisation extends Audit{
    ...
    static async audit(traces:Traces):Promise<Result | SkipResult | undefined>{
        ...
        const findDuplicates = (data:Array<string>):Array<string>=>{
            return Array.from(new Set(data)).filter((value) => data.indexOf(value) !== data.lastIndexOf(value));
        }
        traces.cookies.filter(c=>{
            if(!(hosts.indexOf(c.domain) > -1)) return false
        ...
        if(isAuditApplicable()){
            const duplicatedCookies:Array<string> = findDuplicates(traces.cookies.map(c=>c.name))
            const score = Number(bigCookies.size === 0 && duplicatedCookies.length=== 0);

            const meta = util.successOrFailureMeta(
				CookieOptimisation.meta,
				score
			);
            debug('done');
            
            const cookiesBySize = bigCookies.size > 0 ?
            traces.cookies.filter(c=>Array.from(bigCookies.values()).find(name=>name === c.name)).map(c=>{return {name:c.name, size:c.size}})
            : undefined
