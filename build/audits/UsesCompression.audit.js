...
class UsesCompressionAudit extends audit_1.default {
    static get meta() {
        return {
            id: 'usescompression',
    ...
    static audit(traces) {
        ...
        const compressionRatio = (compressed, uncompressed) => Number.isFinite(compressed) && compressed > 0
        ...
        const resources = traces.record
            .filter(record => {
            if (!APPLICABLE_COMPRESSION_MIME_TYPES.includes(record.response.headers['content-type']))
                return false;
...
exports.default = UsesCompressionAudit;
