...
try {
   ...
   console.error(`${url} is not a valid URL. Make sure to include the scheme (i.e http/https)`);
   process.exit(1);
 }
 ...
 ...(argv['viewport'] || argv['maxnav'] ? {
   connectionSettings : {
     ...(argv['viewport'] === 'mobile' ? {emulatedDevice: {
       name:'mobile',
...
.then((report)=>{
  if (argv['output']) {
     ...
     try {
       ...
       console.log(`Success. Kindly find the latest sustainability report at ${filename}.`)
     } catch (err) {
